    ...
    <div class="mb-6">
        <nav class="text-sm text-gray-500 mb-2">
            <ol class="list-none p-0 flex items-center space-x-2">
                ...
                <li><a href="{{ route($rolePrefix . '.rabies-cases.index') }}" class="hover:text-green-600 transition">Rabies Cases</a></li>
                <li><i class="bi bi-chevron-right text-xs"></i></li>
        ...
        <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
            ...
            <div class="flex items-center gap-3">
                <a href="{{ route($rolePrefix . '.rabies-cases.index') }}" class="px-4 py-2 text-gray-600 hover:text-gray-800 transition">
                ...
                <div class="p-6 space-y-4">
                    ...
                    <div class="border-b border-gray-200 pb-4">
                        ...
                        <div class="space-y-2 text-sm">
                            <a href="{{ route($rolePrefix . '.rabies-bite-reports.show', $case->rabiesReport->id) }}"
                    ...
                    <!-- Status Actions -->
                    @if($case->status === 'open')
                        <form action="{{ route($rolePrefix . '.rabies-cases.complete', $case->id) }}" method="POST">
                            @csrf
                            @method('PUT')
                    ...
                    @if($case->status === 'under_investigation')
                        <form action="{{ route($rolePrefix . '.rabies-cases.complete', $case->id) }}" method="POST">
                            ...
                            <button type="submit" class="w-full inline-flex justify-center items-center gap-2 px-4 py-3 bg-green-600 text-white font-medium rounded-lg hover:bg-green-700 transition">
                                <i class="bi bi-check-all"></i> Close Case
                            </button>
                        </form>
                    @endif

                    @if($case->status !== 'closed')
                        <a href="{{ route($rolePrefix . '.rabies-cases.index') }}?status={{ $case->status }}"
