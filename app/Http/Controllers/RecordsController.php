...
class RecordsController extends Controller
{
    ...
    public function dashboard()
    {
        ...
        $recentAnimals = Pet::with(['owner', 'barangay'])->latest()->take(5)->get();
    ...
    public function storePet(Request $request)
    {
        ...
        if (!empty($request->client_id)) {
            $ownerId = $request->client_id;
        } elseif (!empty($request->owner_name)) {
            // Create a new citizen user for the owner
            $owner = User::create([
                'name' => $request->owner_name,
                'email' => 'petowner_' . time() . '@example.com',
                'password' => bcrypt('password123'),
                'role' => 'citizen',
                'contact_number' => $request->owner_contact ?? null,
            ]);
            $ownerId = $owner->id;
        }
    ...
    public function search(Request $request)
    {
        ...
        // Search animals
        $animals = Pet::where('name', 'like', "%{$search}%")
            ->orWhere('license_number', 'like', "%{$search}%")
